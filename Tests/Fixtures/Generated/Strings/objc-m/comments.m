...
@implementation LocComments : NSObject
+ (NSString*)alertMessage {
    return tr(@"LocComments", @"alert__message");
}
+ (NSString*)alertTitle {
    return tr(@"LocComments", @"alert__title");
}
+ (NSString*)applesCountWithValue:(NSInteger)p1
{
    return tr(@"LocComments", @"apples.count", p1);
}
+ (NSString*)bananasOwnerWithValues:(NSInteger)p1 :(id)p2
{
    return tr(@"LocComments", @"bananas.owner", p1, p2);
}
@end
